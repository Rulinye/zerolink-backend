...
# all three deliverables, not just the main one.
...
.PHONY: build linux linux-amd64 linux-arm64 test run lint clean dev-db tidy \
        linux-amd64-server linux-amd64-admin linux-amd64-import \
        linux-arm64-server linux-arm64-admin linux-arm64-import

build: $(SERVER_BIN) $(ADMIN_BIN) $(IMPORT_BIN)  ## host build
...
# linux-amd64 / linux-arm64 each fan out to three independent targets so make
# evaluates them all even on a fresh checkout.
linux-amd64: linux-amd64-server linux-amd64-admin linux-amd64-import
...
linux-arm64: linux-arm64-server linux-arm64-admin linux-arm64-import
